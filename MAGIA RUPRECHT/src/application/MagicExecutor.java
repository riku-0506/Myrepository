	...
	public static void cast(CustomMagic magic, Character caster,
	    ...
	    for (PrimitiveMagic part : magic.getComponents()) {
	        ...
	        switch (part.getEffectType()) {
	            ...
	            // ---------------- DEBUFF ----------------
	            case DEBUFF -> {
	                UnifiedBuff debuff = createBuffFromMagic(part, false);
	                if (debuff == null) break;

	                for (Character target : targets) {
	                    target.applyBuff(debuff); // ログは applyBuff が出す
	                    // 追加でまとめログを出したいなら debuff.getLabel() を使う：
	                    logs.add(target.getName() + "にデバフ「" + debuff.getLabel() + "」付与");
	                }
	            }
	            ...
	            case STATUS -> {
	                for (Character target : targets) {
	                    StatusEffect effect = part.getStatusEffectEnum();
	                    if (effect != null) {
	                        boolean success = target.tryApplyStatus(effect, caster, effect.getElement());
	                        logs.add(target.getName() + "に" + effect.getLabel() + (success ? "成功" : "失敗"));

	                        if (success && !target.isPlayer()) {
	                            if (controller != null) {
...
//	        System.out.println(">>> MagicExecutor.cast END reached (Runnable版)");
//	        if (onFinished != null) {
//	            System.out.println(">>> MagicExecutor.cast calling onFinished (Runnable版)");
//	            onFinished.run();
//	        }
