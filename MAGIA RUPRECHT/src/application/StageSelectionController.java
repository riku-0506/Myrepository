...
public class StageSelectionController {
    ...
    @FXML private RadioButton stage5;
    @FXML private Text Stage5Name;
    @FXML private Rectangle Stage5bg;

    private static String selectedStage; // ← 保持するステージ名
    private static boolean StageEXFlag = true;
    ...
    public void initialize() {
        ...
        stage5.setUserData("StageEX.fxml");
    ...
    private void StageEXFlagboolean() {
    	try (Connection conn = DBManager.getConnection()) {
            String sql = "SELECT stageEXflag FROM game_state";
            try (PreparedStatement stmt = conn.prepareStatement(sql);
                 ResultSet rs = stmt.executeQuery()) {
            	while (rs.next()) {
                    int flag = rs.getInt("stageEXflag");
                    if(flag == 0) {
                    	StageEXFlag = true;
                    	stage5.setOpacity(0);
                    	Stage5Name.setVisible(false);
                        Stage5bg.setVisible(false);
                        
                    }else {
                    	StageEXFlag = false;
                    	stage5.setOpacity(1);
                    	Stage5Name.setVisible(true);
                        Stage5bg.setVisible(true);
                    }
                }
            }
    ...
    private void StageDecision(ActionEvent event) {
        ...
        if (selected != null) {
            ...
            if(StageEXFlag && selectedStage == "StageEX.fxml") {
            	setStageEXDB();
            }
    ...
    private void setStageEXDB() {
        try (Connection conn = DBManager.getConnection()) {
            String sql = "UPDATE game_state SET stageEXflag = 1";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                stmt.executeUpdate(); // ← これ！
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
