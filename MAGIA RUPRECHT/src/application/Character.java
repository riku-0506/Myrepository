...
public class Character {
 ...
 // 経験値加算とレベルアップ判定
    ...
    public int getModifiedAttack() {
        ...
        // 状態異常補正（感電）
        if (statusTurns.containsKey(StatusEffect.PARALYZE)) {
 ...
 // バフを付与（重複制御付き・完全一致判定あり）
    ...
    //敵専用
    public boolean applyStatus(StatusEffect effect, Character attacker) {
    ...
    public List<String> getActiveStatusEffectIcons() {
        ...
        // 並び順の優先リスト
        List<StatusEffect> order = List.of(
            StatusEffect.BURN,     // やけど
            StatusEffect.PARALYZE, // 感電
            StatusEffect.FREEZE,   // 凍傷
    ...
    public List<UnifiedBuff> getActiveBuffs() {
        return activeBuffs.stream()
            .sorted(
                Comparator.comparing((UnifiedBuff b) -> b.getType().ordinal())
                          .thenComparing(UnifiedBuff::getRank)
            )
            .toList();
    }
...
//        SEPlayer.play("player_attack.mp3");
    ...
    public void setAI(DataDrivenEnemyAI ai) { this.ai = ai;}
    public DataDrivenEnemyAI getAI() { return ai;}
